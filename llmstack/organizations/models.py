...
class Organization(models.Model):
    ...
    admin_email = models.EmailField(
        max_length=100,
        help_text="Email of the admin of the organization",
    )
...
class OrganizationSettings(models.Model):
    ...
    disabled_api_backends = models.ManyToManyField(
        "apiabstractor.ApiBackend",
        blank=True,
        related_name="disabled_api_backends",
    )
    ...
    max_app_visibility = models.PositiveSmallIntegerField(
        default=AppVisibility.PUBLIC,
        choices=AppVisibility.choices,
        help_text="Maximum app visibility for the organization",
    )
    allow_user_keys = models.BooleanField(
        default=True,
        help_text="Whether to allow users to provide their own API keys or not",
    )
    # TODO: move API keys to a separate model since they are common between
    ...
    azure_openai_endpoint = models.CharField(
        max_length=256,
        default=None,
        help_text="Azure OpenAI endpoint to use with Azure openai processor",
        null=True,
        blank=True,
    )
    ...
    vectorstore_weaviate_url = models.CharField(
        max_length=256,
        default=None,
        help_text="Vectorstore URL to use with Vectorstore backend",
        null=True,
        blank=True,
    )
    vectorstore_weaviate_api_key = models.CharField(
        max_length=256,
        default=None,
        help_text="Vectorstore API key to use with Vectorstore backend",
        null=True,
        blank=True,
    )
    vectorstore_weaviate_text2vec_openai_module_config = models.JSONField(
        default=None,
        help_text="Text2Vec OpenAI module config to use with Vectorstore backend",
        null=True,
        blank=True,
    )
    ...
    use_azure_openai_embeddings = models.BooleanField(
        default=False,
        help_text="Whether to use Azure OpenAI embeddings or not",
    )
    embeddings_api_rate_limit = models.PositiveIntegerField(
        default=300,
        help_text="Rate limit for embeddings requests in requests/min",
    )
    embeddings_batch_size = models.PositiveIntegerField(
        default=20,
        help_text="Batch size for embeddings requests",
    )
    ...
    def get_vendor_key(self, attrname):
        if hasattr(self, attrname):
            ...
            if encrypted_key and attrname in [
