...
class PostgresConnection(_Schema):
    ...
    port: int = Field(description='Port number to connect to the Postgres instance')
...
class PostgresDatabaseSchema(DataSourceSchema):
    connection: Optional[PostgresConnection] = Field(description='Postgres connection details')
...
class PostgresDataSource(DataSourceProcessor[PostgresDatabaseSchema]):
    ...
    def __init__(self, datasource: DataSource):
        ...
        if self.datasource.config and 'data' in self.datasource.config:
            config_dict = PostgresConnectionConfiguration().from_dict(self.datasource.config, self.datasource.profile.decrypt_value)
    ...
    def similarity_search(self, query: str, **kwargs) -> List[dict]:
        ...
        result = pg_client.process(PostgresReaderInput(sql=query), configuration=self._reader_configuration).documents[0].content_text
