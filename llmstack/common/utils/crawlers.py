...
class PromptlyScrapyPlaywrightDownloadHandler(ScrapyPlaywrightDownloadHandler):
        ...
        async with self.browser_launch_lock:
            if not hasattr(self, "browser"):
                ...
                self.browser = await self.browser_type.connect(
                    ws_endpoint=self.browser_cdp_url,
                    **self.browser_cdp_kwargs,
                )
    ...
    async def _create_browser_context(
        ...
        context.on(
            "close",
            self._make_close_browser_context_callback(
                name,
                persistent,
                remote,
                spider,
...
class URLSpider(CrawlSpider):
    ...
    def get_text_from_html(self, html_response):
        text_data = [
            text.strip().rstrip()
            for text in html_response.xpath(
    ...
    def _parse_response_data(self, response):
        ...
        if html_content:
            page_data = {
                ...
                "hrefs": [
                    link
                    for link in Selector(text=html_content, type="html")
                    .css(
                        "a::attr(href)",
                    )
                    .getall()
