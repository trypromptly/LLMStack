...
class OpenAIChatCompletionsAPIProcessorConfiguration(
    ...
    logit_bias: Optional[Dict[str, Any]] = Field(
        ...
        + "the bias is added to the logits generated by the model prior to sampling. The exact effect will vary per model, but values between -1 and 1 should decrease or increase likelihood of selection; values like -100 or 100 should result in a ban or exclusive selection of the relevant token.\n",
    )
    stream: Optional[bool] = False
    function_call: Optional[Union[str, Dict]] = Field(
        default=None,
        description='Controls how the model responds to function calls. "none" means the model does not call a function, and responds to the end-user. '
        + '"auto" means the model can pick between an end-user or calling a function. Specifying a particular function via {"name":\\ "my_function"} forces the model to call that function. "none" is the default when no functions are present. "auto" is the default if functions are present.',
    )
    ...
    def _get_api_request_payload(
        ...
        for message in input_json["messages"] + input_json["chat_history"]:
            ...
            if (
                ...
                and "name" in message["function_call"]
                and not message["function_call"]["name"]
            ):
                del message["function_call"]
...
class OpenAIImageEditsProcessor(
    OpenAIAPIProcessor[
...
class OpenAIImageVariationsProcessor(
    ...
    def _get_api_request_payload(
        ...
        configuration: OpenAIImageVariationsProcessorConfiguration,
    ) -> dict:
        configuration_json = json.loads(configuration.json())
        return {
            "image": input.image,
...
class OpenAIEmbeddingsProcessor(
    OpenAIAPIProcessor[
        OpenAIEmbeddingsProcessorInput,
        OpenAIEmbeddingsProcessorOutput,
        OpenAIEmbeddingsProcessorConfiguration,
    ],
):
    def _get_api_url(self) -> str:
    ...
    def _get_api_request_payload(
        ...
        configuration: OpenAIEmbeddingsProcessorConfiguration,
    ) -> dict:
        return {
