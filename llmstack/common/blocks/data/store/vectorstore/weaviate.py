...
class Weaviate(VectorStoreInterface):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        def check_batch_result(
            ...
            time_took_to_create_batch = DEFAULT_BATCH_SIZE * (
                self._client.batch.creation_time / self._client.batch.batch_size
            )

            time.sleep(
                max(
                    DEFAULT_BATCH_SIZE / NUM_OBJECTS_PER_SECOND - time_took_to_create_batch,
                    0,
                ),
            )
        ...
        if configuration.weaviate_rw_api_key is not None:
            headers["authorization"] = "Bearer " + configuration.weaviate_rw_api_key

        # TODO: Read these from configuration
        connect_timeout = settings.VECTOR_DATABASES.get(
            "default",
        ).get("CONNECT_TIMEOUT", 5)
    ...
    def get_document_by_id(
        ...
        try:
            ...
            return Document(
                content_key,
                result["properties"].get(content_key, None),
        ...
        **kwargs: Any,
        ...
        try:
            ...
            query_response = (
                ...
                .with_additional(additional_metadata_properties)
                .do()
            )
        except Exception as e:
        ...
        if (
            ...
            or index_name not in query_response["data"]["Get"]
        ):
            logger.error(
                "Invalid response from Weaviate: %s Index Name: %s" % query_response,
                index_name,
            )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if whereFilter:
                query_obj = query_obj.with_where(whereFilter)
            query_response = (
                query_obj.with_hybrid(
                    query=document_query.query,
                    alpha=document_query.alpha,
                )
                .with_limit(
                    document_query.limit,
                )
