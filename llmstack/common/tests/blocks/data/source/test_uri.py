...
class UriTextLoaderTestCase(unittest.TestCase):
    ...
    def test_data_url_process_txt(self):
        with open(f'{self.fixtures_dir}/sample.txt', 'r') as file_p:
            ...
            result = Uri().process(
                input=UriInput(
                    uri=f'data:{mime_type};name=sample.txt;base64,{base64_encoded_data.decode("utf-8")}',
    ...
    def test_data_url_process_csv(self):
        with open(f'{self.fixtures_dir}/sample.csv', 'r') as file_p:
            ...
            result = Uri().process(
                input=UriInput(
                    uri=f'data:{mime_type};name=sample.csv;base64,{base64_encoded_data.decode("utf-8")}',
                    env=DataSourceEnvironmentSchema(
                        openai_key=self.openai_api_key)),
                configuration=UriConfiguration())
            self.assertEqual(len(result.documents), 1)
    ...
    def test_data_url_process_rtf(self):
        with open(f'{self.fixtures_dir}/sample.rtf', 'r') as file_p:
            ...
            result = Uri().process(
                input=UriInput(
                    uri=f'data:{mime_type};name=sample.rtf;base64,{base64_encoded_data.decode("utf-8")}',
                    env=DataSourceEnvironmentSchema(
                        openai_key=self.openai_api_key)),
                configuration=UriConfiguration())
            self.assertEqual(len(result.documents), 1)
    ...
    def test_data_url_process_json(self):
        with open(f'{self.fixtures_dir}/sample.json', 'r') as file_p:
            ...
            result = Uri().process(
                input=UriInput(
                    uri=f'data:{mime_type};name=sample.json;base64,{base64_encoded_data.decode("utf-8")}',
                    env=DataSourceEnvironmentSchema(
                        openai_key=self.openai_api_key)),
                configuration=UriConfiguration())
            self.assertEqual(len(result.documents), 1)
    ...
    def test_data_url_process_pdf(self):
        with open(f'{self.fixtures_dir}/sample.pdf', 'rb') as file_p:
            ...
            result = Uri().process(
                input=UriInput(
                    uri=f'data:{mime_type};name=sample.pdf;base64,{base64_encoded_data.decode("utf-8")}',
                    env=DataSourceEnvironmentSchema(
                        openai_key=self.openai_api_key)),
                configuration=UriConfiguration())
            self.assertEqual(len(result.documents), 1)
    ...
    def test_http_url_process_pdf(self):
        ...
        self.assertEqual(
            result.documents[0].metadata['mime_type'], 'application/pdf')
        self.assertEqual(
            result.documents[0].metadata['file_name'],
