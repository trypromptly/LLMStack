import logging
from llmstack.apps.app_session_utils import create_app_session_data, get_app_session_data
...
class AppProcessorRunner(AppRunner):
    ...
    def _get_processor_actor_configs(self, processor_id):
        ...
        processor_cls = ApiProcessorFactory.get_api_processor(
            app_processor['processor_slug'], app_processor['provider_slug'],
        )
        app_session_data = get_app_session_data(
            self.app_session, app_processor,
        )
        ...
        processor_actor_configs.append(
            ActorConfig(
                name=app_processor['id'], template_key='processor', actor=processor_cls, kwargs={
                    'id': app_processor['id'],
                    'env': vendor_env,
