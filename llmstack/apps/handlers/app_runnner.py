...
class AppRunner:
    ...
    def __init__(
        ...
        if not self.app_run_request_user.is_anonymous:
            if (
                self.app_run_request_user
                and self.app_run_request_user.email
                and len(
                    self.app_run_request_user.email,
                )
                > 0
            ):
                ...
                self.app_run_request_user
                and self.app_run_request_user.username
                and len(self.app_run_request_user.username) > 0
            ):
    ...
    def _is_app_accessible(self):
        ...
        if (
            self.app.visibility == AppVisibility.ORGANIZATION
            and self.app_owner_profile.organization
            != Profile.objects.get(
                user=self.app_run_request_user,
            ).organization
        ):
            raise Exception("App not found")

        if (
            self.app.visibility == AppVisibility.PRIVATE
            and self.app_run_request_user != self.app.owner
            and self.app_run_request_user.email not in self.app.read_accessible_by
            and self.app_run_request_user.email not in self.app.write_accessible_by
        ):
            raise Exception("App not found")
    ...
    def _get_processor_actor_configs(self):
        ...
        if self.app_data and "processors" in self.app_data:
            ...
            processors = [
                x.exit_endpoint
                for x in self.app.run_graph.all().order_by(
                    "id",
                )
    ...
    def _start(
        ...
        try:
            ...
            if input_actor and output_actor:
                ...
                output_iter = (
                    output_actor.get_output().get() if not self.stream else output_actor.get_output_stream().get()
                )
    ...
    def _get_csp(self):
        ...
        if self.app.is_published:
            ...
            if (
                self.web_config
                and "allowed_sites" in self.web_config
                and len(
                    self.web_config["allowed_sites"],
                )
                > 0
            ):
                csp = "frame-ancestors " + " ".join(self.web_config["allowed_sites"])
        return csp
