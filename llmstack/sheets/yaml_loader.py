...
def load_sheet_formula_templates():
    formula_templates = {}
    if not hasattr(settings, "SHEET_FORMULA_TEMPLATES_DIR"):
        return formula_templates

    for dir in settings.SHEET_FORMULA_TEMPLATES_DIR:
        if not os.path.isdir(dir) or not os.path.exists(dir):
            continue
        for filename in os.listdir(dir):
            if filename.endswith(".yml"):
                with open(os.path.join(dir, filename), "r") as file:
                    template = yaml.safe_load(file)
                    formula_templates[template["slug"]] = template
    return formula_templates


def get_sheet_formula_template_by_slug(slug: str):
    formula_templates = load_sheet_formula_templates()
    return formula_templates.get(slug)
