...
class AbstractProfile(models.Model):
    ...
    def weaviate_url(self):
        ...
        if settings.VECTOR_DATABASES.get('default') and settings.VECTOR_DATABASES.get(
                'default').get('ENGINE') == 'weaviate':
    ...
    def connections(self):
        return {
            k: json.loads(
                self.decrypt_value(v)) for k,
            v in self._connections.items()} if self._connections else {}

    def get_connection(self, id):
        return json.loads(self.decrypt_value(self._connections.get(
            id))) if self._connections and self._connections.get(id) else None
    ...
    def get_connection_by_type(self, connection_type_slug):
        return [
            json.loads(
                self.decrypt_value(v)) for v in self._connections.values() if json.loads(
                self.decrypt_value(v))['connection_type_slug'] == connection_type_slug]

    def _vendor_key_or_promptly_default(self, attrname, api_key_value):
        if attrname == 'azure_openai_api_key':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_AZURE_OPENAI_API_KEY
        elif attrname == 'openai_key':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_OPENAI_API_KEY
        elif attrname == 'stabilityai_key':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_DREAMSTUDIO_API_KEY
        elif attrname == 'cohere_key':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_COHERE_API_KEY
        elif attrname == 'forefrontai_key':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_FOREFRONTAI_API_KEY
        elif attrname == 'elevenlabs_key':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_ELEVENLABS_API_KEY
        elif attrname == 'google_service_account_json_key':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_GOOGLE_SERVICE_ACCOUNT_JSON_KEY
        elif attrname == 'aws_secret_access_key':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_AWS_SECRET_ACCESS_KEY
        elif attrname == 'aws_default_region':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_AWS_DEFAULT_REGION
        elif attrname == 'azure_openai_endpoint':
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_AZURE_OPENAI_ENDPOINT
            ...
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_LOCALAI_API_KEY
            ...
            return self.decrypt_value(
                api_key_value) if api_key_value else settings.DEFAULT_ANTHROPIC_API_KEY
