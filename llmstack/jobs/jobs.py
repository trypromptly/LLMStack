...
def run_app_subtask(app_id, input_data):
    ...
    request = RequestFactory().post(
        f'/api/apps/{app_id}/run', data=request_input_data, format='json')
...
def post_run_app_task(task_run_log_uuid, input_index, status, response, job):
    if task_run_log_uuid:
        ...
        if status == TaskStatus.SUCCESS and response['status_code'] == 200:
            task_run_log.result[input_index] = SubTaskResult(
                status=TaskStatus.SUCCESS, output=response['data']).dict()
        else:
            task_run_log.result[input_index] = SubTaskResult(
                status=TaskStatus.FAILURE, error=response).dict()
    ...
    if input_index + 1 < len(input_data):
        ...
        time_remaining_to_schedule_next_task = max(
            (settings.TASK_RUN_DELAY - time_elapsed), 1)
        logger.info(
...
def post_upsert_datasource_task(
    ...
    if task_run_log_uuid:
        ...
        if status == TaskStatus.SUCCESS and response['status_code'] == 200:
            task_run_log.result[input_index] = SubTaskResult(
                status=TaskStatus.SUCCESS, output='success').dict()
        else:
            task_run_log.result[input_index] = SubTaskResult(
                status=TaskStatus.FAILURE, error=response).dict()
        task_run_log.save()
