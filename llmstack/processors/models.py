...
class Share(models.Model):
    ...
    owner = models.ForeignKey(
        ...
        null=False,
        on_delete=models.DO_NOTHING,
        help_text="Owner of the shared entity",
    )
    name = models.CharField(
        max_length=100,
        default="Untitled",
        help_text="Name of the share",
    )
    code = models.CharField(
        max_length=8,
        help_text="Code to share the endpoint",
    )
    api_backend = models.ForeignKey(
        ApiBackend,
        on_delete=models.PROTECT,
        help_text="API backend for this share",
    )
    param_values = models.JSONField(
        blank=True,
        null=True,
        default=None,
        help_text="Endpoint param values",
    )
    prompt = models.TextField(
        blank=True,
        null=True,
        default="",
        help_text="Endpoint prompt",
    )
    prompt_values = models.JSONField(
        blank=True,
        null=True,
        default=None,
        help_text="Endpoint prompt values",
    )
    input = models.JSONField(
        blank=True,
        null=False,
        default=dict,
        help_text="Endpoint input values",
    )
    output = models.JSONField(
        blank=True,
        null=False,
        default=dict,
        help_text="Endpoint output values",
    )
    template_values = models.JSONField(
        blank=True,
        null=True,
        default=None,
        help_text="Endpoint input template values",
    )
    config_values = models.JSONField(
        blank=True,
        null=False,
        default=dict,
        help_text="Endpoint config values",
    )
    ...
    response = models.ForeignKey(
        ...
        default=None,
        null=True,
        help_text="Response to share",
    )
    tags = models.ManyToManyField(ShareTag)
    created_on = models.DateTimeField(auto_now_add=True)
    is_archived = models.BooleanField(
        default=False,
        help_text="Is this share archived",
    )
    is_private = models.BooleanField(
        default=False,
        help_text="Is this share private",
        blank=True,
    )
