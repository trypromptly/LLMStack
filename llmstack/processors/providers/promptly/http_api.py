...
class ParameterType(Schema):
    ...
    location: ParameterLocation = Field(
        default=ParameterLocation.PATH, title='Parameter location', description='Where to pass the parameter')
...
class RequestBodyParameterType(Schema):
    ...
    type: FieldType = Field(default=FieldType.STRING,
                            title='Parameter type', description='Parameter data type')
    required: bool = Field(default=True, title='Is parameter required')
    description: Optional[str] = Field(title='Parameter description')
...
class HttpAPIProcessorInput(Schema):
    input_data: Optional[str] = Field(
        description='Input for parameter keys as JSON', advanced_parameter=False, widget='textarea')
...
class RequestBody(Schema):
    ...
    payload: Optional[str] = Field(
        title='Request body payload', default=None,
...
class HttpAPIProcessorConfiguration(Schema):
    ...
    request_body: Optional[RequestBody] = Field(
        default=None, advanced_parameter=False, description='Request body')
    ...
    timeout: Optional[float] = Field(
        default=5, advanced_parameter=True, ge=0, le=100, description='Timeout in seconds')
...
class PromptlyHttpAPIProcessor(ApiProcessorInterface[HttpAPIProcessorInput, HttpAPIProcessorOutput, HttpAPIProcessorConfiguration]):
    ...
    def process(self):
        ...
        if self._config.connection_id:
            ...
            if connection_type == 'credentials':
                ...
                if type_slug == 'basic_authentication':
                    ...
                    headers[connection['configuration']['header_key']
                            ] = connection['configuration']['api_key']
        ...
        url = hydrate_input(
            url, {'_parameters': path_params, '_connection': connection_configuration})
        ...
        if self._config.request_body and self._config.request_body.payload:
            ...
            body = hydrate_input(
                body, {'_parameters': body_data, '_connection': connection_configuration})
