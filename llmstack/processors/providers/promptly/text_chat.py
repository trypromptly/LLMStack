...
{% endif %}''')
    ...
    def session_data_to_persist(self) -> dict:
        return {'chat_history': self._chat_history[-self._config.chat_history_limit:]
                if self._config.chat_history_limit > 0 else [], 'context': self._context}
    ...
    def _search_datasources(self, input):
        ...
        def fetch_datasource_docs(datasource_uuid):
            ...
            try:
                ...
                if len(
                        self._chat_history) > 0 and self._config.chat_history_in_doc_search > 0:
    ...
    def process(self) -> dict:
        ...
        self._chat_history.append({'role': 'assistant',
                                   'content': output['answer'] if isinstance(output,
                                                                             dict) else output.answer},
                                  )
