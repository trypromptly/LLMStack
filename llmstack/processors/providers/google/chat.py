...
class ChatConfiguration(ApiProcessorSchema):
    ...
    generation_config: GenerationConfig = Field(
        advanced_parameter=False, default=GenerationConfig())
...
class ChatProcessor(
    ...
    def process(self) -> dict:
        ...
        message_params = [{'text': self._input.system_message}
                          ] if self._input.system_message else []
        ...
        for chunk in response:
            ...
            tool_calls = list(map(lambda x: ToolCall(type='function', function=FunctionCall(
                name=x['name'], parameters=json.dumps(x['args']))), function_calls))
        ...
        if self._config.retain_history:
            history = history + messages + \
                [{'parts': [{'text': output.prediction.content}], 'role': 'model'}]
