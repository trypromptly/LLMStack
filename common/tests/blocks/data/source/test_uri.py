...
from common.blocks.data.source.uri import UriInput, UriConfiguration, Uri


class UriTextLoaderTestCase(unittest.TestCase):
    def setUp(self) -> None:
        self.fixtures_dir = __file__.replace('test_uri.py', '../../../fixtures')
    ...
    def test_http_url_process_pdf(self):
        result = Uri().process(
            input=UriInput(uri='https://arxiv.org/pdf/1706.03762.pdf', env=None),
            configuration=UriConfiguration()
            )
        self.assertTrue(len(result.documents) > 0)
        self.assertEqual(result.documents[0].metadata['mime_type'], 'application/pdf')
        self.assertEqual(result.documents[0].metadata['file_name'], 'https://arxiv.org/pdf/1706.03762.pdf')
